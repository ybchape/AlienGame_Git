...
func _ready() -> void:
	...
	energia_actual = 3 - RunManager.penalizacion_energia
	# reset energia a 3
	RunManager.penalizacion_energia = 0
	...
	barra_vida_jugador.max_value = RunManager.vida_maxima 
	barra_vida_jugador.value = RunManager.vida_jugador
	
	# 2. Preparar mano (Mezclar y Robar 5)
	mazo_principal = RunManager.mazo_actual.duplicate()
...
func _jugar_carta(nodo, datos):
	...
	if energia_actual >= coste_real:
		...
		if datos.daño > 0:
			...
			if RunManager.bonus_doble_dano:
				dano_a_realizar*= 2 #aplica el multiplicador de daño x2
				RunManager.bonus_doble_dano = false
		...
		if datos.escudo > 0:
			...
			var escudo_final = datos.escudo - RunManager.penalizacion_escudo
...
# --- LÓGICA DE RECIBIR EL DAÑO (Es igual para todos) ---
	if dano_enemigo > 0:
		if escudo_actual >= dano_enemigo:
			...
			escudo_actual = 0
			RunManager.vida_jugador -= dano_restante
		...
		if RunManager.penalizacion_escudo >0: 
			RunManager.penalizacion_escudo = 0
...
func iniciar_nuevo_turno_jugador():
	...
	if GameManager.esta_en_descontrol:
		...
		print("El virus te consume: -5 PV. Vida restante: ", RunManager.vida_jugador)
		if RunManager.vida_jugador <=0:
...
func mostrar_resultado(gano: bool):
	...
	if gano:
		# Recuperamos vida
		RunManager.vida_jugador = clamp(RunManager.vida_jugador + 8,0, RunManager.vida_maxima)
